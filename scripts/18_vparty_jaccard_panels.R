...
panel_figure <- function(cells, summary, universe_label, fill_max) {
  ...
  ggplot(drawn, aes(x = col_i, y = row_i, fill = jaccard)) +
    ...
    labs(
      ...
      x = sprintf("Populism bin (1 = lowest, %d = highest)", N_BINS),
      y = sprintf("Anti-pluralism bin (1 = lowest, %d = highest)", N_BINS)
    ) +
...
for (u in names(all_cells)) {
  ...
  ggsave(
    file.path(out_dir, sprintf("jaccard_panels_%s.png", u)),
    fig,
    width = 13, height = 6.2, dpi = 150
  )
  cat(sprintf("Saved jaccard_panels_%s.png\n", u))
